...
import {
  ...
  UserGroupIcon,
  ClockIcon,
  VideoCameraIcon, // آیکون جدید برای تماس تصویری
} from "@heroicons/react/24/outline";
import moment from "moment-jalaali";
import CreateMeetingModal from "../components/meetings/CreateMeetingModal";
import { Link, useNavigate } from "react-router-dom"; // افزودن useNavigate
...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case "active_call": // وضعیت جدید برای جلسات با تماس فعال
      return {
...
const Meetings = () => {
  ...
  const [showVideoCall, setShowVideoCall] = useState(false); // برای نمایش/مخفی کردن مودال ویدیو کال
  const [currentMeetingId, setCurrentMeetingId] = useState<number | null>(null); // برای ذخیره ID جلسه فعلی
  ...
  // تابع برای شروع تماس تصویری
  const handleStartVideoCall = (meetingId) => {
    // می‌توانید از هر دو روش استفاده کنید:
    
    // روش 1: نمایش مودال ویدیو کال
    setCurrentMeetingId(meetingId);
    setShowVideoCall(true);
    
    // روش 2: هدایت به صفحه جداگانه ویدیو کال
    // navigate(`/meetings/${meetingId}/video-call`);
  };
  
  // تابع برای شروع تماس سریع (بدون جلسه خاص)
                ...
                >
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      ${getStatusColor(meeting.status).text} flex items-center gap-1`}
                      >
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          ...
                          ${getStatusColor(meeting.status).text} flex items-center gap-1 w-fit`}
                        >
                          {getStatusColor(meeting.status).icon}{" "}
                          {getStatusText(meeting.status)}
                        </span>
